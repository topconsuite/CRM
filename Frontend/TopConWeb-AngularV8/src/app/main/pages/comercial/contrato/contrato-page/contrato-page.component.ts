...
import { ObraSimplesDTO, construcaoCivilTipoAlvara, EConstrucaoCivilTipoAlvara } from 'app/classes/obra/obra';
...
import { EObraTracoStatus, ObraTracoStatusResponse } from 'app/classes/traco/traco-status';
import { TributacaoReforma } from 'app/classes/tributacao-reforma/tributacao-reforma';
import { TributacaoReformaService } from 'app/services/tributacao-reforma.service';
...
@Component({
  ...
  styleUrls: ['./contrato-page.component.scss']
})
export class ContratoPageComponent implements OnInit, OnChanges {

  public static self: ContratoPageComponent;
  ...
  get construcaoCivilTipoAlvara(): number[] {
    let codigos: number[] = [];
    construcaoCivilTipoAlvara.forEach(status => {
      codigos.push(status.codigo);
    })
    return codigos;
  };
  ...
  tributacoesIS: TributacaoReforma[] = [];
  tributacoesIBS: TributacaoReforma[] = [];
  tributacoesCBS: TributacaoReforma[] = [];
  ...
  constructor(
      ...
      private _tributacaoReformaService: TributacaoReformaService,
      private location: Location
    ) {
    ContratoPageComponent.self = this;
    ...
    _tributacaoReformaService.listarTodosProducao("IS").then(
        tributacoesIS => { this.tributacoesIS = [{ id: 0 }, ...tributacoesIS] },
        error => { this.tributacoesIS = [] }
    );

    _tributacaoReformaService.listarTodosProducao("IBS").then(
        tributacoesIBS => { this.tributacoesIBS = [{ id: 0 }, ...tributacoesIBS] },
        error => { this.tributacoesIBS = [] }
    );

    _tributacaoReformaService.listarTodosProducao("CBS").then(
        tributacoesCBS => { this.tributacoesCBS = [{ id: 0 }, ...tributacoesCBS] },
        error => { this.tributacoesCBS = [] }
    );
  ...
  ngOnInit() {
    ...
    this.faturamentoForm = this._formBuilder.group({
      utilizaDadosFaturamento: [''],
      utilizaEnderecoFaturamento: ['']
    });
    this.cobrancaForm = this._formBuilder.group({
      utilizaDadosCobranca: [''],
      utilizaEnderecoCobranca: ['']
    });
    ...
    this.reajusteForm = this._formBuilder.group({});
    this.obraFrenteForm = this._formBuilder.group({});

    let idUsina = parseFloat(this._route.snapshot.paramMap.get('idUsina'));
    let ano = parseFloat(this._route.snapshot.paramMap.get('anoProposta'));
    let numero = parseFloat(this._route.snapshot.paramMap.get('numeroProposta'));
  ...
  get demaisServicosValorMinimoValidator(): ICustomValidator {
    var _self = ContratoPageComponent.self;
  ...
  get temDireitoAlteracaoFimVigencia(): boolean {
    const { _userService } = this;

    return _userService.temDireitoAplicativo('WEB6110', '');
  }
  ...
  getObrigaNumeroContratoAnterior() {
    return this.parametroProposta.obrigaNumeroContratoAnterior;
  }
  ...
  tributacaoReformaFormatter = (model: TributacaoReforma): string => model ? model.id.toString() : '';
  ...
  construcaoCivilTipoAlvaraFormatter = (model: number): string => {
    if (model===null || model===undefined || isNaN(model)) return '';
    return this.construcaoCivilTipoAlvara.includes(model) ? construcaoCivilTipoAlvara.filter(e => e.codigo===model)[0].descricao.toUpperCase() : '';
  };
  ...
  get utilizaDadosFaturamento(): boolean {
    return this.proposta.utilizaDadosFaturamento;
  }
  set utilizaDadosFaturamento(value: boolean) {
    this._utilizaDados('faturamento', 'utilizaDadosFaturamento', value);
  }

  get utilizaEnderecoFaturamento(): boolean {
    return this.proposta.utilizaEnderecoFaturamento;
  }
  set utilizaEnderecoFaturamento(value: boolean) {
    this._utilizaEndereco('faturamento', 'utilizaEnderecoFaturamento', value);
  }

  get utilizaDadosCobranca(): boolean {
    return this.proposta.utilizaDadosCobranca;
  }
  set utilizaDadosCobranca(value: boolean) {
    this._utilizaDados('cobranca', 'utilizaDadosCobranca', value);
  }

  get utilizaEnderecoCobranca(): boolean {
    return this.proposta.utilizaEnderecoCobranca;
  }
  set utilizaEnderecoCobranca(value: boolean) {
    this._utilizaEndereco('cobranca', 'utilizaEnderecoCobranca', value);
  }
        ...
        .then(interveniente => {
          ...
          if (interveniente) {
            ...
            if (vendedor !== 0 && vendedor !== this.proposta.vendedor.codigo) {
              var self = ContratoPageComponent.self;
  ...
  closeModal() {
    let self = ContratoPageComponent.self;
  ...
  async removeTraco(traco: ObraTraco) {
    ...
    if (this.contratoTracoReajustesFiltered().filter(t => t.obraTracoSequencia === traco.sequencia).length > 0) {
      self.proposta.obra.obraTracos.find(t => t.sequencia === traco.sequencia).ativo = 'N';
      self.proposta.obra.obraTracos.find(t => t.sequencia === traco.sequencia).inativo = true;
      return;
    }
  ...
  valorTotalTracos(): number {
    let total: number = 0;
    this.proposta.obra.obraTracos.forEach(item => {
      total += ((item.precoReajustadoAtual != 0 ? item.precoReajustadoAtual : item.m3PrecoProposto) * item.m3Quantidade);
    });
  ...
  descontoPercentualTraco(traco: ObraTraco): number {
    ...
    if(!this.possuiContratoReajuste(traco)){
      ...
      traco.descontoPercentual = (1 - (traco.precoReajustadoAtual / (traco.m3PrecoAjustado === 0 ? traco.m3PrecoTabela : traco.m3PrecoAjustado))) * 100;
    }
  ...
  mensagemExclusaoTraco(traco: ObraTraco): string{
    if (this.contratoTracoReajustesFiltered().filter(t => t.obraTracoSequencia === traco.sequencia).length > 0)
      return 'O traço possui um reajuste cadastrado. Não será possível excluí-lo, apenas inativá-lo. Confirma a inativação?';

    return 'Confirma exclusão do traço?';
  }
  ...
  carregaTracoNumeracoesProduto(dispararChange: boolean = true) {
    ...
    if (self.proposta.obra.usinaEntrega) {
      ...
      .then(numeracoesProduto => {
          self.numeracoesProduto = numeracoesProduto.filter(t => t !== null && t.status != EObraTracoStatus.CustoVirtual);
  ...
  bombaTipoChange(bombaTipo: CadastroGeral) {
    if (!this.bomba.bombaTipo || !bombaTipo || this.bomba.bombaTipo.codigo !== bombaTipo.codigo) {
      ...
      this.verificaTipoBombaJaIncluso();
      if (this.tipoBombaJaIncluso) {
        this.removeValoresBomba();
      }
      else this.carregaBombaPreco();
    }
  }
  ...
  removeValoresBomba(){
    this.bomba.bombaTipo = null;

    this.bomba.m3TabelaAte = 0;
    this.bomba.taxaMinimaPrecoTabela = 0;
    this.bomba.m3PrecoTabela = 0;
    
    this.bomba.horaTabelaAte = 0;
    this.bomba.horaTaxaMinimaPrecoTabela = 0;
    this.bomba.horaPrecoTabela = 0;
  ...
  onFaturamentoNext() { this.saveProgress(); }
  onCobrancaNext() { this.saveProgress(); }
  ...
  onReajusteContratoNext(){ this.saveProgress(); }
  ...
  onTaxasExtrasNext() {
    ...
    if (!self.proposta.obra.obraPagamentos || self.proposta.obra.obraPagamentos.length === 0) {
      ...
      if (self.proposta.obra.condicaoPagamento.codigo !== pagamento.condicaoPagamento.codigo && pagamento.valorApropriado > 0 && pagamento.idAprovacao === '') {
        ...
        novoPagamento.valor = valorTotal;
  ...
  onFaturamentoPrevious() { this.saveProgress(); }
  onCobrancaPrevious() { this.saveProgress(); }
  onResponsavelSolidarioPrevious() { this.saveProgress(); }
  onObraPrevious() { this.saveProgress(); }
  onConcretoPrevious() { this.saveProgress(); }
  onReajusteContratoPrevious() { this.saveProgress(); }
  ...
  contratoBombaReajusteUltimasVigencias(bomba: ObraBomba): ContratoBombaReajuste[] {
    ...
    var bombasFiltradas = this.proposta.obra.contrato.contratoBombaReajustes
      .filter((t => t.bombaTipoCodigo === bomba.bombaTipo.codigo) && (t => t.obraBombaReajusteSequencia === bomba.sequencia));
  ...
  verificaTipoBombaJaIncluso() {
    let self = ContratoPageComponent.self;  
    
    self.tipoBombaJaIncluso = false
    
    self.proposta.obra.obraBombas.forEach(bomba => {
      if (self.bomba.bombaTipo && bomba.bombaTipo && bomba.bombaTipo.codigo == self.bomba.bombaTipo.codigo && bomba.sequencia != self.bomba.sequencia) {
        self.tipoBombaJaIncluso = true;
        ...
        self.dialog.open(AlertDialogComponent, {
          ...
          data: {
            title: 'TopConWeb',
            message: 'A inclusão deste tipo de bomba não é permitida, pois o mesmo já se encontra na proposta atual.'
          }           
        });
